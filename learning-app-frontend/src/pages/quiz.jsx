...
function Quiz({ mastery, setMastery, profile }) {
  ...
  if (!profile && !isRetestMode) {
    return (
      <div className="quiz-container">
        ...
        <p className="quiz-meta">We need a few details before starting your quiz.</p>
        <button className="quiz-next-btn enabled" onClick={() => navigate('/onboarding')}>
          Go to Onboarding
        </button>
      </div>
    )
  ...
  if (loading) {
    return (
      <div className="quiz-container">
        <p className="quiz-meta">Loading your {subjectName || subject} quiz...</p>
      </div>
    )
  }
  ...
  if (phase === 'complete') {
    return (
      <div className="quiz-container">
        ...
        <p className="quiz-meta">You've worked through every subtopic in {subjectName}. Check your Roadmap for the full picture.</p>
      </div>
    )
  }

  const currentSubtopic = bank[subtopicIndex]
  const questionsForLevel = currentSubtopic.questions[level] || currentSubtopic.questions.Intermediate || []
  const currentQuestion = questionsForLevel[questionIndex]
  const isSelected = !!answers[currentQuestion?.id]
  const progressPercent = ((subtopicIndex + (questionIndex + 1) / questionsForLevel.length) / bank.length) * 100
  ...
  const advanceToNextSubtopic = (score) => {
    setMastery({ ...mastery, [currentSubtopic.id]: Math.max(mastery[currentSubtopic.id] || 0, score) })
  ...
  const handleNext = () => {
    ...
    if (score >= PASS_THRESHOLD) {
      ...
      setMastery({ ...mastery, [currentSubtopic.id]: Math.max(mastery[currentSubtopic.id] || 0, score * 0.5) })
      setPhase('fail')
    }
  }
  ...
  if (phase === 'fail') {
    return (
      <div className="quiz-container">
        ...
        <p className="quiz-meta">You didn't quite clear this section — review the resource below, then retake the quiz.</p>
  ...
  return (
    <div className="quiz-container">
      ...
      <p className="quiz-meta">
        ...
        {currentSubtopic.name} · Question {questionIndex + 1} of {questionsForLevel.length}
      ...
      <div className="quiz-options">
        {currentQuestion.options.map((option) => (
          <button
            key={option}
            className={answers[currentQuestion.id] === option ? 'quiz-option selected' : 'quiz-option'}
      ...
      <button
        ...
        onClick={handleNext}
      >
        {questionIndex === questionsForLevel.length - 1 ? 'Finish Subtopic' : 'Next'}
      </button>
    </div>
  )
}
